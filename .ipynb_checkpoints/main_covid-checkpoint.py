...
# TIMESTAMP = dt.astimezone(tzutc_8)
...
parser.add_argument('--shellnum', default=0, type=int, 
                    help='The number')
...
#             transforms.CenterCrop(224),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        writer.add_scalars('Accuracy' + str(N_S), {
            'Train': acc1_tr,
            'Val': acc1
        }, epoch)
        writer.flush()
        
        writer.add_scalars('Loss' + str(N_S), {
            'Train': loss_tr,
            'Val': loss_te
        }, epoch)
        writer.flush()
...
#         print('_____________________________ACC1_train: ', acc1)
...
#             target = target.to(device)  # missing line from original code
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True)
            target = target.cuda(args.gpu, non_blocking=True)
...
#             print('_____________________________ACC1_test: ', acc1)
...
def save_checkpoint(state, is_best, epoch, filename=args.resumedir + 'checkpoint-' + TIMESTAMP + '.pth.tar'):
